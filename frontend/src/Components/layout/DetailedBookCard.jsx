                ...
                {books?.map((book, index) => (
                    <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden flex">
                        ...
                        <div className="w-2/3 p-4">
                            ...
                            <p className="text-gray-700 text-sm block md:hidden">{book?.description?.length > 80 ? `${book.description.substring(0, 80)}...` : book?.description}</p>
                            <button onClick={() => navigate(`/booksDetail/${book.id}`)} className='text-sm mt-4 bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-2 rounded'>بینینی کتێب</button>
                        </div>
                    </div>
                ))}
            </div>
