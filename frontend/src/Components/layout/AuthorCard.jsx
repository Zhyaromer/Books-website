  ...
  return (
    <div>
      <Card className={`overflow-hidden transition-all h-[300px] duration-300 ease-in-out hover:shadow-xl hover:scale-[1.03] bg-gradient-to-b from-white to-slate-50 dark:from-slate-900 dark:to-slate-800 ${className}`}>
        <div className="absolute inset-0 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 opacity-30 rounded-xl transition-opacity duration-300 ease-in-out group-hover:opacity-50"></div>
        ...
        <CardContent className="text-center pb-5 relative z-10">
          <div className="flex justify-center space-x-3 mt-2">
            <button onClick={() => window.location.href = `/author/${id}`} className="px-4 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-sm font-medium transition-all duration-300 ease-in-out hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-blue-200 dark:hover:border-blue-700 transform hover:-translate-y-0.5">
              بینینی نووسەر
            </button>
          </div>
        </CardContent>
